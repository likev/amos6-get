...
let getRH = function (content) {
	...
	stationList.each((index, item) => {
		...
		let time = tdList.eq(1).text().slice(0, 17),
			rain = tdList.eq(3).text(),
			T = tdList.eq(2).text(),
			windVal = tdList.eq(27).text(),
			windDir = tdList.eq(26).text(),
			rh = tdList.eq(13).text(),
			localP = tdList.eq(18).text();
...
let getPage = function ({ t = '921', countyno = 'O2672', startdate = '20170918200000', endate = '20170919080000', response } = {}, callback = getRH) {
